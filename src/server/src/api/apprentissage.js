...
apprentissage.get(
  ...
  async (req, res) => {
    ...
    const apprentissageData = await (isSiren(id)
      ? apprentissage.getBySiren(id)
      : apprentissage.getBySiret(id));

    const apprentissageMaxYear = await apprentissage.getMaxYear();

    if (!apprentissageMaxYear) {
      res.status(404).json({
        message: "No apprentissage for this SIRET or SIREN",
      });
      return;
    }
    ...
    const compactApprentissageData = apprentissageData.reduce(
      (acc, apprentissage) => {
