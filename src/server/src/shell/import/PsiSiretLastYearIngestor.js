...
class PsiSiretLastYearIngestor extends PsiIngestor {
  _runPsqlCopy() {
    ...
    const psqlImportQuery = `${this.psql} "\
      ...
      SELECT ${columns} \
      FROM import_csv \
      ON CONFLICT ON CONSTRAINT psi_siret_siret_key DO UPDATE SET salaries_annee_precedente = excluded.salaries_annee_precedente;"`;

    return execSync(psqlImportQuery);
  }
}

module.exports = PsiSiretLastYearIngestor;
